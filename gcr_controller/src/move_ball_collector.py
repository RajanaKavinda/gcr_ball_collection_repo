...
class BallCollectorJointController(Node):
    def __init__(self):
        super().__init__('ball_collector_joint_controller')
        self.client = ActionClient(self, FollowJointTrajectory, '/ball_collector_joint_controller/follow_joint_trajectory')
        self.client.wait_for_server()
        self.get_logger().info("Ball Collector Joint Controller Ready. Enter goals to move the collector.")
...
def main(args=None):
    ...
    node = BallCollectorJointController()

    try:
        while True:
            try:
                position = float(input("\nEnter desired ball collector position between (0 - pi/2): "))
